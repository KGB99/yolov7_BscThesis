...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', train_dir=None, labels_dir=None):
...
class LoadImagesFromLabelFile:  # for inference
    def __init__(self, filepath, img_dir, img_size=640, stride=32):
        #p = str(Path(path).absolute())  # os-agnostic absolute path
        #if '*' in p:
        #    files = sorted(glob.glob(p, recursive=True))  # glob
        #elif os.path.isdir(p):
        ...
        # made to read from my own ground truth labels
        files = []
        with open(filepath, 'r') as f:
            gt_dict = json.load(f)
        for camera in gt_dict:
            for img_id in gt_dict[camera]:
                files.append(img_dir + '/' + gt_dict[camera][img_id]['img']['file_name'])
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='', train_dir=None, labels_dir=None):
        self.img_size = img_size
        ...
        self.label_files = img2label_paths(cache.keys(), train_dir, labels_dir)  # update
        if single_cls:
